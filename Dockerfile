...
RUN mkdir -p /usr/src/gret_geonode
...
COPY . /usr/src/gret_geonode/
WORKDIR /usr/src/gret_geonode
...
RUN chmod +x /usr/src/gret_geonode/tasks.py \
    && chmod +x /usr/src/gret_geonode/entrypoint.sh
...
ENTRYPOINT service cron restart && /usr/src/gret_geonode/entrypoint.sh
