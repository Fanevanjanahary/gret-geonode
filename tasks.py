...
def update(ctx):
    ...
    try:
        current_allowed = ast.literal_eval(os.getenv('ALLOWED_HOSTS') or \
                                           "['{public_fqdn}', '{public_host}', 'localhost', 'django', 'gret_geonode',]".format(**envs))
    except ValueError:
        current_allowed = []
...
--settings={0}".format(_localsettings()), pty=True)
    ctx.run("python manage.py loaddata /usr/src/gret_geonode/fixtures/initial_data.json \
--settings={0}".format(_localsettings()), pty=True)
...
def _localsettings():
    settings = os.getenv('DJANGO_SETTINGS_MODULE', 'gret_geonode.settings')
    return settings
